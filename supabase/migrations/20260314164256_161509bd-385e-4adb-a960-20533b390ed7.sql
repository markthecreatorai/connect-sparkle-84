...
DECLARE
  ...
  _w_bal numeric;
  _new_w_bal numeric;
  -- Upline VIP plan commission percentages
  _upline_vip_level int;
  _upline_plan RECORD;
  _pct numeric;
BEGIN
  ...
  -- 5) Distribute task commissions using UPLINE'S VIP plan percentages
  -- Commission rule: upline receives commission based on THEIR OWN vip_plan's commission_a/b/c_pct * task reward
  SELECT * INTO _ref FROM referral_tree WHERE user_id = _user_id;

  -- Level A → personal wallet (direct sponsor)
  IF _ref.level_a_referrer IS NOT NULL THEN
    -- Get upline's VIP level and their plan's commission_a_pct
    SELECT COALESCE(p.vip_level, 0) INTO _upline_vip_level FROM profiles p WHERE p.id = _ref.level_a_referrer AND p.is_active = true;
    IF _upline_vip_level IS NOT NULL THEN
      SELECT * INTO _upline_plan FROM vip_plans WHERE level = _upline_vip_level;
      IF FOUND THEN
        _pct := COALESCE(_upline_plan.commission_a_pct, 0);
        _comm_amount := ROUND(_reward * _pct, 2);
        IF _comm_amount > 0 THEN
          SELECT id, COALESCE(balance, 0) INTO _w_id, _w_bal FROM wallets WHERE user_id = _ref.level_a_referrer AND wallet_type = 'income';
          IF _w_id IS NOT NULL THEN
            ...
            INSERT INTO transactions (user_id, type, wallet_type, amount, balance_after, description, status, metadata)
            VALUES (_ref.level_a_referrer, 'referral_task_commission', 'income', _comm_amount, _new_w_bal,
              'Comissão tarefa subordinado Nível A', 'completed',
              jsonb_build_object('from_user', _user_id, 'level', 'A', 'task_reward', _reward,
                'upline_vip_level', _upline_vip_level, 'commission_pct', _pct, 'calculated_amount', _comm_amount));
          END IF;
        END IF;
      END IF;
    END IF;
  ...
  -- Level B → income wallet (2nd level)
  IF _ref.level_b_referrer IS NOT NULL THEN
    SELECT COALESCE(p.vip_level, 0) INTO _upline_vip_level FROM profiles p WHERE p.id = _ref.level_b_referrer AND p.is_active = true;
    IF _upline_vip_level IS NOT NULL THEN
      SELECT * INTO _upline_plan FROM vip_plans WHERE level = _upline_vip_level;
      IF FOUND THEN
        _pct := COALESCE(_upline_plan.commission_b_pct, 0);
        _comm_amount := ROUND(_reward * _pct, 2);
        IF _comm_amount > 0 THEN
          ...
          IF _w_id IS NOT NULL THEN
            ...
            UPDATE wallets SET balance = _new_w_bal, updated_at = now() WHERE id = _w_id;
            INSERT INTO transactions (user_id, type, wallet_type, amount, balance_after, description, status, metadata)
            VALUES (_ref.level_b_referrer, 'referral_task_commission', 'income', _comm_amount, _new_w_bal,
              'Comissão tarefa subordinado Nível B', 'completed',
  ...
  -- Level C → income wallet (3rd level)
  IF _ref.level_c_referrer IS NOT NULL THEN
    SELECT COALESCE(p.vip_level, 0) INTO _upline_vip_level FROM profiles p WHERE p.id = _ref.level_c_referrer AND p.is_active = true;
    IF _upline_vip_level IS NOT NULL THEN
      SELECT * INTO _upline_plan FROM vip_plans WHERE level = _upline_vip_level;
      IF FOUND THEN
        _pct := COALESCE(_upline_plan.commission_c_pct, 0);
        _comm_amount := ROUND(_reward * _pct, 2);
        IF _comm_amount > 0 THEN
          ...
          IF _w_id IS NOT NULL THEN
            ...
            UPDATE wallets SET balance = _new_w_bal, updated_at = now() WHERE id = _w_id;
            INSERT INTO transactions (user_id, type, wallet_type, amount, balance_after, description, status, metadata)
            VALUES (_ref.level_c_referrer, 'referral_task_commission', 'income', _comm_amount, _new_w_bal,
              'Comissão tarefa subordinado Nível C', 'completed',
