...
CREATE OR REPLACE FUNCTION public.complete_daily_task(_user_id uuid, _task_id uuid, _task_number int)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT id, COALESCE(balance, 0) INTO _wallet FROM wallets WHERE user_id = _user_id AND wallet_type = 'personal';
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'wallet_not_found');
  END IF;
  _new_balance := _wallet.balance + _reward;
